...
function pcbPinGridEndpointForShape(c, shape, profile, board, hole, normal, requestedWidth) {
  ...
  const inwardNormal = unitNormal(normal);
  const endpointNormal = inwardNormal ? { x: -inwardNormal.x, y: -inwardNormal.y } : normal;
  const edgeAnchor = pcbHoleBoardEdgeAnchor(shape, board, hole);
  const escapePolygon = gridAlignedPcbPinEscapePolygon(c, shape, profile, hole, width);
  const ringPolygons = pinContactRingPolygons(shape, profile, hole, width);
  const contactPoint = pcbPinContactEndpointPoint(shape, board, hole, endpointNormal, width);
  const contactGridPoint = pinContactBoundaryGridPoint(c, ringPolygons, contactPoint, endpointNormal)
    ?? bestPinContactGridPoint(c, ringPolygons, contactPoint, endpointNormal)
    ?? snapToMainGrid(contactPoint, c);
  const frameGridPoint = outerPcbPinGridPoint(c, [escapePolygon], edgeAnchor, inwardNormal)
    ?? snapToMainGrid(edgeAnchor, c);
  const point = contactGridPoint;
  return {
    point,
    ...
    contactGridPoint,
    frameGridPoint,
    selectedEdge: shortPcbPinSelectedEdge(point, endpointNormal, c, shape.id, hole.id),
    source: "pcb-pin",
    pcbShapeId: shape.id,
    pinId: hole.id,
    pinLabel: hole.label ?? hole.id,
...
function pcbPinEndpointLeadPath(endpoint) {
  const contact = endpoint?.contactGridPoint ?? endpoint?.contactPoint;
  const point = endpoint?.frameGridPoint ?? endpoint?.boundaryPoint ?? endpoint?.point;
...
function pinContactBoundaryGridPoint(c, polygons, contactPoint, normal) {
  ...
  const boundaryCoord = horizontalNormal
    ? (unit.x >= 0 ? bounds.x + bounds.w : bounds.x)
    : (unit.y >= 0 ? bounds.y + bounds.h : bounds.y);
  let best = null;
  for (const x of xs) {
    for (const y of ys) {
      const candidate = { x, y };
      const onBoundaryDrift = horizontalNormal ? Math.abs(x - boundaryCoord) : Math.abs(y - boundaryCoord);
      if (onBoundaryDrift > pitch * 0.75) continue;
      const perpendicularDrift = horizontalNormal ? Math.abs(y - contactPoint.y) : Math.abs(x - contactPoint.x);
      if (perpendicularDrift > Math.max(pitch * 1.25, Number(c.pcbPinContactWidth ?? 1.6))) continue;
      const near = validPolygons.some((polygon) => pointInExpandedBounds(candidate, polygonBounds(polygon), Number(c.beadWidth ?? 0.42) * 1.8));
      if (!near) continue;
      const outward = (candidate.x - contactPoint.x) * unit.x + (candidate.y - contactPoint.y) * unit.y;
      const score = onBoundaryDrift * 8 + perpendicularDrift + distance(candidate, contactPoint) * 0.25 - outward * 0.4;
