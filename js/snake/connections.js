...
import {
  ...
  snakeGridPoints,
  usedSnakeBucket,
} from "./conflicts.js?v=auto-workflow-pin-endpoint-final-v1-20260827";
import {
  effectiveGuidePointsForEndpoints,
  withCurrentLeadLength,
} from "./endpoints.js?v=auto-workflow-pin-endpoint-final-v1-20260827";
import { buildTpuSnakePath } from "./candidates.js?v=auto-workflow-pin-endpoint-final-v1-20260827";
...
function normalizeConnectionEndpoint(endpoint, c, deps) {
  ...
  if (endpoint.source === "pcb-pin" && resolved?.point) {
    return {
      ...endpoint,
      ...resolved.point,
      ...
      contactGridPoint: resolved.contactGridPoint ?? endpoint.contactGridPoint ?? null,
      frameGridPoint: resolved.frameGridPoint ?? endpoint.frameGridPoint ?? null,
...
export function buildConnectionSnakePaths(c, deps, connections, materialResult, layerIndex, planners) {
  ...
  for (const { connection, index } of planningOrder) {
    ...
    const candidateDeps = isSvgGuideConnection && !avoidT0Blocks
      ? {
        ...localDeps,
...
function resolveSnakeConnectionEndpoints(connection, sourceBucket, c, deps, planners) {
  const endpoints = (connection.endpoints ?? []).slice(0, 2).map((endpoint) => {
    ...
    return normalizeResolvedEndpoint(withCurrentLeadLength({ ...endpoint, rawPoint }, c));
  });
...
function normalizeResolvedEndpoint(endpoint) {
  const point = endpoint.boundaryPoint ?? endpoint;
  return {
    ...endpoint,
    x: Number(point.x),
    y: Number(point.y),
    boundaryPoint: { x: Number(point.x), y: Number(point.y) },
  };
}

function blockedBucketForConnection(bucket, connection) {
  const ownEndpoints = (connection?.endpoints ?? [])
    .filter((endpoint) => endpoint && Number.isFinite(Number(endpoint.x)) && Number.isFinite(Number(endpoint.y)));
  const ownPinEndpoints = ownEndpoints
  ...
  if (ownPins.size === 0 && ownEndpoints.length === 0) return bucket;
  ...
  const keepPlanningSegment = (segment, direction) => {
    ...
    return !ownEndpoints.some((endpoint) => segmentTouchesConnectionEndpoint(segment, direction, endpoint));
  };
  return {
...
function segmentTouchesConnectionEndpoint(segment, direction, endpoint) {
  const point = endpoint?.boundaryPoint ?? endpoint;
