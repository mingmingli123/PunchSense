...
export function pcbPinContactFramePaths(c, shapes, connections = []) {
  return pcbPinContactRegions(c, shapes, connections)
...
function pcbPinGridEndpointForShape(c, shape, profile, board, hole, normal, requestedWidth) {
  ...
  const gridPoint = bestPinContactGridPoint(c, pinContactRingPolygons(shape, profile, hole, width), contactPoint, normal)
    ?? outerPcbPinGridPoint(c, [escapePolygon], edgeAnchor, normal);
