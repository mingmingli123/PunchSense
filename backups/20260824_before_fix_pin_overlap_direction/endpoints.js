...
export function resolveEndpointOnBoundaryGrid(endpoint, rows, deps, c) {
  ...
  if (!normal || !rows?.length) return endpoint;
...
function endpointLeadChain(endpoint, c) {
  if (endpoint?.source === "pcb-pin") {
    const contact = pcbPinInteriorAnchor(endpoint, c);
    const boundary = endpoint?.boundaryPoint ?? endpoint;
...
export function endpointInteriorAnchor(endpoint, c) {
  if (endpoint?.source === "pcb-pin") return pcbPinInteriorAnchor(endpoint, c);
...
function pcbPinInteriorAnchor(endpoint, c) {
  ...
  if (!normal) return contactPoint;
  const dx = boundaryPoint.x - contactPoint.x;
  const dy = boundaryPoint.y - contactPoint.y;
  const distance = Math.hypot(dx, dy);
  if (distance > 0.001) {
    const pitch = Math.max(0, Number(c?.pitch ?? 0));
    const width = Math.max(0.2, Number(c?.pcbPinContactWidth ?? 1.6));
    const overlap = Math.min(
      Math.max(distance, Math.min(width, Math.max(1.0, pitch * 0.5))),
      Math.max(width, pitch || width),
    );
    return {
      x: boundaryPoint.x + (dx / distance) * overlap,
      y: boundaryPoint.y + (dy / distance) * overlap,
    };
  }
  const horizontalNormal = Math.abs(normal.x) >= Math.abs(normal.y);
  return horizontalNormal
    ? { x: contactPoint.x, y: boundaryPoint.y }
    : { x: boundaryPoint.x, y: contactPoint.y };
}
