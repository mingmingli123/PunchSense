...
function endpointLeadPoints(endpoint) {
  if (endpoint?.source === "pcb-pin") {
    ...
    if (contact && Number.isFinite(Number(contact.x)) && Number.isFinite(Number(contact.y))) {
      points.push({ x: Number(contact.x), y: Number(contact.y) });
    }
    if (boundary && Number.isFinite(Number(boundary.x)) && Number.isFinite(Number(boundary.y))) {
      points.push({ x: Number(boundary.x), y: Number(boundary.y) });
    }
    return points;
  }
