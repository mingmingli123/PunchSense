import { pathLength } from "./stats.js?v=auto-workflow-guides-v2-20260823";
import {
  ...
  snakeGridPoints,
  usedSnakeBucket,
} from "./conflicts.js?v=auto-workflow-guides-v2-20260823";
import {
  effectiveGuidePointsForEndpoints,
  resolveEndpointOnBoundaryGrid,
  withCurrentLeadLength,
} from "./endpoints.js?v=auto-workflow-guides-v2-20260823";
import { buildTpuSnakePath } from "./candidates.js?v=auto-workflow-guides-v2-20260823";
...
export function buildConnectionSnakePaths(c, deps, connections, materialResult, layerIndex, planners) {
  ...
  for (const { connection, index } of planningOrder) {
    ...
    const guidedDeps = {
      ...candidateDeps,
      blockedSnakeBucket: isSvgGuideConnection ? usedBucket : localBlockedBucket,
    };
    ...
    if (!snake.points?.length && planningBlockedBucket) {
    ...
    if (planners.enableGlobalFallback && shouldTryGlobalSnakeFallback(snake, localConfig, localDeps)) {
    ...
    const blockedConflictBucket = isSvgGuideConnection ? localHardBlockedBucket : localBlockedBucket;
